...
public class QuizActivity extends AppCompatActivity implements ValueEventListener{
    ...
    void onUpdateQuestion(DataSnapshot snapshot) {
        ...
        answer = snapshot.child("quiz").child(keyQuiz).child(keyQuestion).child("answer").getValue() + "";
    ...
    private void clickAnswer(int number) {
        ...
        String ans = "" + (char) ('A' + number - 1);
            ...
            FSInstance.db.collection("answer1").document(keyQuestion).get().addOnCompleteListener(
                    new OnCompleteListener<DocumentSnapshot>() {
                        ...
                        public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                            ...
                            s.put(MainActivity.USER_ID,ans.equals(answer) ? "" + (100 - 2 * s.keySet().size()): "0");
    ...
    public void onDataChange(@NonNull DataSnapshot snapshot) {
        if (snapshot.child("flag").hasChild("answer")
        ...
        if (QuizStartActivity.isAnswer && "init".equals("" + snapshot.child("flag").child("status").getValue())) {
